...
mod tests {
    ...
    fn test_splitter() {
        ...
        crate::runtime::Builder::new_current_thread().build().unwrap().block_on(fut);
    }
    #[test]
    #[cfg_attr(miri, ignore)]
    fn test_pseudo_text() {
        ...
        data.extend(
            std::iter::repeat(0b1010_1010).take(DEFAULT_MAX_BUF_SIZE - checked_count + 1),
        );
        ...
        assert_eq!(writer.write_history.iter().copied().sum::< usize > (), data.len());
