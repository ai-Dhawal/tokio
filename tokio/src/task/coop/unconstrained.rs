...
pin_project! {
    #[doc = " Future for the [`unconstrained`](unconstrained) method."]
    #[cfg_attr(docsrs, doc(cfg(feature = "rt")))] #[must_use =
    "Unconstrained does nothing unless polled"] pub struct Unconstrained < F > { #[pin]
    inner : F, }
}
    ...
    F: Future,
{
    ...
    cfg_coop! {
        ...
        Self::Output > { let inner = self.project().inner; crate
        ::task::coop::with_unconstrained(|| inner.poll(cx)) }
    }
    cfg_not_coop! {
        fn poll(self : Pin <& mut Self >, cx : & mut Context <'_ >) -> Poll <
        Self::Output > { let inner = self.project().inner; inner.poll(cx) }
    }
}
