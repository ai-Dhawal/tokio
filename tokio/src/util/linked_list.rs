...
unsafe impl<L: Link> Send for LinkedList<L>
where
    L::Target: Send,
{}
unsafe impl<L: Link> Sync for LinkedList<L>
where
    L::Target: Sync,
{}
...
cfg_io_driver_impl! {
    pub (crate) struct DrainFilter <'a, L : Link, F > { list : &'a mut LinkedList < L >,
    filter : F, curr : Option < NonNull < L::Target >>, } impl < L : Link > LinkedList <
    L > { pub (crate) fn drain_filter < F > (& mut self, filter : F) -> DrainFilter <'_,
    L, F > where F : FnMut(& L::Target) -> bool, { let curr = self.head; DrainFilter {
    curr, filter, list : self, } } } impl <'a, L : Link, F > Iterator for DrainFilter
    <'a, L, F > where F : FnMut(& L::Target) -> bool, { type Item = L::Handle; fn next(&
    mut self) -> Option < Self::Item > { while let Some(curr) = self.curr { self.curr =
    unsafe { L::pointers(curr).as_ref() } .get_next(); if (self.filter) (unsafe { & mut *
    curr.as_ptr() }) { return unsafe { self.list.remove(curr) }; } } None } }
}
cfg_taskdump! {
    impl < L : Link > LinkedList < L > { pub (crate) fn for_each < F > (& mut self, mut f
    : F) where F : FnMut(& L::Handle), { let mut next = self.head; while let Some(curr) =
    next { unsafe { let handle = ManuallyDrop::new(L::from_raw(curr)); f(& handle); next
    = L::pointers(curr).as_ref().get_next(); } } } }
}
feature! {
    ...
    #[doc = " at the guard node itself."] pub (crate) struct GuardedLinkedList < L : Link
    > { #[doc = " Pointer to the guard node."] guard : NonNull < L::Target >, } impl < L
    : Link > LinkedList < L > { #[doc =
    ...
    guard_handle : L::Handle) -> GuardedLinkedList < L > { let guard = L::as_raw(&
    guard_handle); unsafe { if let Some(head) = self.head {
    ...
    GuardedLinkedList { guard } } } impl < L : Link > GuardedLinkedList < L > { fn tail(&
    self) -> Option < NonNull < L::Target >> { let tail_ptr = unsafe { L::pointers(self
    .guard).as_ref().get_prev().unwrap() }; if tail_ptr != self.guard { Some(tail_ptr) }
    else { None } } #[doc =
