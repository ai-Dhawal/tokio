...
impl WorkerMetrics {
    ...
    cfg_metrics_variant! {
        ...
        .as_ref().map(| histogram_builder | histogram_builder.build()); #[cfg(feature =
        "schedule-latency")] { worker_metrics.schedule_latency_histogram = config
        .metrics_schedule_latency_histogram.as_ref().map(| histogram_builder |
        histogram_builder.build()); } worker_metrics } }
    }
    cfg_unstable_metrics! {
